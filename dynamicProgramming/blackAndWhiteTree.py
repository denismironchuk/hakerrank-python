...
def markTreeNoRecur(start, islandNum, processed):
    stack = []
    stack.append(start)
    start.color = 0
    ...
    while (len(stack)):
        ...
        for neigh in nd.neighbours:
            if (not processed[neigh.num]):
                processed[neigh.num] = True
                neigh.color = (nd.color + 1) % 2
                stack.append(neigh)

if __name__ == '__main__':
    #with open('C:\\Users\\dmiro\\inpt.txt', 'r') as f:
        ...
        for num in range(1, n + 1):
            if (not processed[num]):
                ...
                markTreeNoRecur(nodes[num], island, processed)
                island += 1
